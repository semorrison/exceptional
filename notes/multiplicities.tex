...
\begin{align*}
  ...
  \dim(Y_2^*) &= -\frac{[4][5][6][\lambda-6][\lambda+3][3\lambda+3]}
                 {[2][\lambda-1][\lambda][\lambda+1][2\lambda-2][2\lambda-1]}\\
  ...
  \dim(Y_3^*) &= - \frac{[4][5][6][\lambda-6][\lambda+3][\lambda+4][\lambda+5][2\lambda+2][5\lambda+1]}
                     {[2][\lambda-1][\lambda][\lambda+1][2\lambda-2][2\lambda-1][2\lambda][3\lambda-3][3\lambda-2]} \\
  ...
  \dim(C^*) &= \frac{[4][5][6][\lambda-6][\lambda-4][\lambda+4][2\lambda-6][2\lambda+2][2\lambda+4][4\lambda+2]}
                     {[1][\lambda-3][\lambda-1]^2[\lambda][\lambda+1][2\lambda-1][2\lambda+1][3\lambda-3][3\lambda-1]}
\end{align*}
\nn{The dimensions for A and C were calculated (and the earlier cases checked) in \tt{code2/BraidedTrivalent-QES-6-box-dimensions.nb}, based on null space calculations from \tt{toolkit/.../BraidedTrivalent-QES-6-box-verify-braiding.nb}.}
...
\nn{Scott todo: make a table for how these all specialise to the exceptional Lie algebras.}
...
\begin{table}
  ...
  \begin{tabular}{ccccccccccc}
    \toprule
%     &&&&&\multicolumn{5}{c}{Twist eigenvals}\\ \cmidrule(l){6-10}
    & $X_1$ & $X_2$ & $Y_2$ & $Y_2^*$ & $X_3$ & $Y_3$ & $Y_3^*$ & $A$ & $C$ & $C^*$ \\ \midrule
%      $v^{12}$ & $-v^6$ & $1$ & $w^2$ & $v^2w^{-2}$ \\ \midrule
$F_4$ & $e_1$ & $e_2$ & $2e_1$ & $2e_4$ & $2e_3$ & $3e_1$ & $e_3$ & $e_1 + 2 e_4$ & $e_1 + e_2$ & $e_3 + e_4$ \\[3pt]
$G_2$ & $e_2$ & $3e_1$ & $2e_2$ & $2e_1$ & $4e_1$ & $3e_2$ & $e_1$ & $2e_1+e_2$ & $3e_1+e_2$ & $e_1+e_2$ \\[3pt]
$E_7$ & $e_1$ & $e_3$ & $2e_1$ &$e_6$ & $e_4$ & $3e_1$ & $2e_7$ & $e_1 + e_6$ & $e_1+e_3$ & $e_2+e_7$ \\[3pt]
$E_8$ & $e_8$ & $e_7$ & $2e_8$ &$e_1$ & $e_6$ & $3e_8$ & --- & $e_1 + e_8$ & $e_7+e_8$ & $e_2$ \\[3pt]
    \bottomrule
  \end{tabular}
  \medskip
  \caption{Identifications of simples as highest weight irreps of the exceptional Lie algebras}
