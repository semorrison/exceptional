...
the values on the left of Table~\ref{tab:lambda-group}, then the quotient of
$\mathsf{Exc}_{\mathbb{C},\lambda}$ by the negligible elements is equivalent
...
\item If $\lambda$ is one of the special values in
  Table~\ref{tab:lambda-group}, then the quotient of
  $\mathsf{Exc}_{\mathbb{C},\lambda}$ by the negligible elements is
...
\begin{corollary}[Quantum specialization]
  If $\lambda$ is one of the values in Table~\ref{tab:lambda-group}
  and $v$ is not a root of unity, then the quotient of
$\mathsf{QExc}_{\mathbb{C},v,v^\lambda}$ by the negligible elements is
...
$\mathsf{Except}((h))$.
\DPTtodo{Check situation with $\mathsf{Exc}$ vs $\mathsf{Except}$.
  Compare Conjecture 1.7}
...
\DPTtodo{These lines seem too thick}
\begin{align*}
  (\rho_n : \V_n \to \V_n) & = 
    \begin{tikzpicture}[baseline, ultra thick]
...
\nn{Compare to Mironov--Mkrtchyan--Morozov \texttt{1510.05884},
  Westbury \texttt{arXiv:1510.08307} and Mironov--Morozov
  \texttt{arXiv:1511.09077}\dots}
