...
an object $X$ with $\dim \cC_1 = 0$, $\dim{\cC_2} = 1$, and $\dim{\cC_3} = 1$, with a (unique up to rescaling) non-zero element $\tau \in \cC_3$ called `the trivalent vertex', such that the category is generated as a ribbon category by $\tau$.\NStodo{Do we really want nondegenerate?, instead we could just ask $b\neq0$, $d\neq0$}
\end{definition}

Being generated as a ribbon category by $\tau$ is the same as saying that $\cC$ is generated as a pivotal category by $\tau$ and the crossing.  In particular, a trivalent ribbon category need not be trivalent in the sense of \cite{MR3624901}.
...
Combining Propositions \ref{??} and \ref{??}, we have proved all of Theorem \ref{??} except for the situation where $\beta = 0$, which we address now.
...
\; + v^{-4} \;
 ...
 \item $[\alpha:\beta] = [b\left(1-\frac{v^{-3}}{v^{12}-v^{-12}}\right):1]$ and $v = e^{\frac{2 \pi i}{30}}$ 
 \end{itemize}
\end{lemma}
\begin{proof}
...
We now use the QEJacobi relation again to remove the X-term, yielding the QESquare relation.

Take the QESquare relation and subtract its 90-degree rotation, yielding the QECrossing equation.
\end{proof}
...
\begin{lemma} \ref{lem:bplus3alpha}
