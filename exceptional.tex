...
\begin{multline*}
QES(\nn{trefoil}) = 
\frac{v^{12} \left(v^4+1\right) \left(v^{12}-w^2\right)  \left(v^{10} w^2-1\right)}{(w^2-1) (v^2-w^2)} \times \\
\Big(
...
\end{equation*}
\SMtodo{surely the coefficient of the crossing is wrong; two $\alpha$s??}
...
When $b+[3]\alpha$ is non-zero and $v$ is not a $12$th root of unity we can make the change of variables
$$w^2 = \frac{v}{2(b+[3]\alpha)}\left(b \{1\} -\alpha [3] \{5\} + \sqrt{-4 (b+[3]\alpha)^2 + (b \{1\} -[3]\{5\}\alpha)^2} \right)$$
and set $b = 1$ to see relations are the same as the versions in terms of $w$ given earlier.

We then have $$\frac{\alpha}{b} =  -\frac{[\lambda][\lambda-1]}{\{\lambda+2\}\{\lambda-3\}\widetilde{\Phi}_3\widetilde{\Phi}_1}.$$

The restriction that $v$ not be a $12$th root of unity comes from looking at when the denominator $\{\lambda+2\}\{\lambda-3\}\widetilde{\Phi}_3$ vanishes after you plug in the formula for $w$. \SMtodo{I don't understand this remark.}
...
Suppose that the diagrams   
  ...
  \cupcap\;,\qquad\twostrandid\;,
   ...
   \]
are linearly dependent.  Then $\cC$ is one of
\begin{itemize}

 \item the golden category, $SO(3)_q$ (including $OSp(1|2)$ when $q= \pm i$) for some $q$, or $S_3$.  

 \item $SO(3)_q$ for some $q$ (which includes the possibility $OSp(1|2)$ when $q= \pm i$), and there are three distinct relations of the above form, namely (if we normalize $b=1$) $\alpha = -\frac{v-v^{-1}}{v^4-1+v^{-4}}$ for $v$ a sixth root of $q^{-2}$,

 \item or $\Rep S_3$, and there are three distinct relations of the above form (if we normalize $b=1$):
\begin{itemize}
