...
\DPTtodo{Need to either take Karoubi envelope on the
  $\mathsf{Exc}$ side or restrict to tensor products on rep theory
  side. Also in Corollary~\ref{cor:quant-specialization}}
\end{corollary}
...
\DPTtodo{In Table~\ref{tab:lambda-group}, shouldn't it be
  $\mathrm{PO}(8)\rtimes S_3$ instead?}
...
\DPTtodo{Is this conditional on one of the conjectures above?}
\begin{corollary}[Quantum specialization]\label{cor:quant-specialization}
...
semisimple.
\DPTtodo{Modified the conjecture to be a subset of $\mathbb{C}^2$ not $\mathbb{C}$}
\end{conjecture}
