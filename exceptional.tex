...
So long as a certain finite set of polynomials in $\lambda$ are invertible in $R$, then any non-zero $R$-multiple of the empty diagram, the strand, or the trivalent vertex is nonzero in $\mathrm{Exc}_{R,\lambda}$.
\end{conjecture}


The following corollary follows from Conjectures \ref{conj:class-suffic} and \ref{conj:class-consist}.
...
So long as a certain finite set of Laurent polynomials in $v$ and $w$ are invertible in $R$, then any non-zero $R$-multiple of the empty diagram, the single strand, or the trivalent vertex is non-zero in $\mathrm{QExc}_{R,v,w}$.  In particular, any two ways of applying the above relations to a closed diagram to return a rational function in $v$ and $w$ will yield the same rational function.
\end{conjecture}


\begin{conjecture}[Quantum specialization]
If $\lambda$ is one of $-5, -4, -3, -2, -3/2,1,2/3,1/2, 1/3, 1/5$, and $v$ is not a root of unity, then the quotient of $\mathrm{Exc}_{\mathbb{C},v,v^\lambda}$ by the negligible elements is equivalent to the category of representations of the quantum versions of trivial group, $O(1 | 2)$, $\mathrm{PSL}(2)$, $\mathrm{PSL}(3) \rtimes \mathbb{Z}/2$, $G_2$, $\mathrm{PO}(8) \rtimes \mathbb{Z}/3$, $F_4$, $E_6 \rtimes \mathbb{Z}/2$, $E_7$, and $E_8$, respectively.
\end{conjecture}
