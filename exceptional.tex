...
\begin{remark}
  There have been several parameters used for the exceptional
  series. For several natural parameters, there is an involution. For
  reference, here is a list of the various parameters.
  \begin{itemize}
  ...
  \item the dual Coxeter number $h^\vee = 6/\mu$
    \cite{MR1952563};
\smargin{DPT: Actually, $w$ as stated doesn't appear in the published
  paper by Vogel, only in the preprint version. I haven't sorted out
  the published one yet.}
  \item the number $w$ in Equation~\eqref{eq:classical-except}, with
    $b=\mu(1+\mu)/2$ \cite{MR2769234}; and
