...
\nn{This $v$ is unique up to...}

\begin{theorem}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category with $\dim{\cC_4} \leq 5$, let $v$ be as in the previous theorem, and further suppose that $d$, $b$,$[6]$, and $[5]$ are nonzero.  Then $\widetilde{\Phi}_8 = \{2\}$, $\alpha$, and $b+[3]\alpha$ are also non-zero, and we have the following relations:
...
\begin{equation} \ngon[45]{4} + \frac{\alpha \widetilde{\Phi}_{12} (b+[3]\alpha)}{\widetilde{\Phi}_1 \widetilde{\Phi}_8 \widetilde{\Phi}_3} \braidcross + \frac{y_1}{\widetilde{\Phi}_8 \widetilde{\Phi}_3} \drawI + \frac{y_2}{\widetilde{\Phi}_8 \widetilde{\Phi}_3} \drawH + \frac{y_3}{\widetilde{\Phi}_1 \widetilde{\Phi}_8 \widetilde{\Phi}_3} \cupcap + \frac{y_4}{\widetilde{\Phi}_1 \widetilde{\Phi}_8 \widetilde{\Phi}_3} \twostrandid =0
\end{equation}
...
If we now make the following change of variables then the $\alpha$ versions of these relations turn into the $w$ versions given earlier

$$w^2 = \frac{v}{2(b+[3]\alpha)}\left(b \{1\} -\alpha [3] \{5\} + \sqrt{-4 (b+[3]\alpha)^2 + (b \{1\} -[3]\{5\}\alpha)^2} \right).$$

\begin{corollary}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category with $\dim{\cC_4} \leq 5$, let $v$ be as above, and further suppose that $d$, $b$,$[6]$, and $[5]$ are nonzero.  Then there's a $w \in \mathbb{C}$ and an evaluation functor $$\mathrm{QExc}_{\mathbb{C},v,w} \rightarrow \cC$.
\end{corollary}
