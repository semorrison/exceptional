...
This includes all prime knots and nearly all prime links of at most 12 crossings,
and many non-algebraic examples.
...
The usual normalization is $b = \Psi_3(q) \Psi_{8}(q)$ which makes the coefficients of the pentagon relation both $1$ as in \cite{MR1403861} and simplifies the crossing relation.  In \cite{MR3624901} as usual we normalized so that $b=1$.
...
Unless $s$ is $-1$, this says that $\scalebox{0.5}{\braidcross}$ simplifies.  So we split into two cases.

If $\scalebox{0.5}{\braidcross}$ simplifies, then we need only solve for all braidings for
