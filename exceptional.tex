...
\item If $\lambda$ is one of the special values in
  ...
  $\mathsf{Exc}^D_{\mathbb{C},\lambda}$ by negligible elements is
  equivalent to the category of representations of the corresponding
  quantum supergroup at an appropriate value of $q$ \nn{add reference to table once it's there}.
\end{itemize}
\NStodo{Dylan left a few todos here, which I think I have to-done, except the note at the end.  
That table has to come later because it depends on our quantum group conventions.}
\end{conjecture}
...
product group.  For example $(PSU(3) \rtimes \mathbb{Z}/2\mathbb{Z})_q$ denotes the
...
$PSU(3) \rtimes \mathbb{Z}/2\mathbb{Z}$.
