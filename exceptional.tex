...
\NStodo{Already known for torus knots, should add references somewhere}  For
...
\DPTtodo{I think this is too strong: it could be, eg, that there is
  some torsion in the quotient $\mathsf{Exc}_{R,\lambda}$. What we
  really want is that $\mathsf{Exc}_{R,\lambda}$ has a free quotient
  or some such}
...
= 1$, and $\dim{\cC_3} = 1$, \NStodo{Dimension doesn't make sense over general R} with a (unique up to rescaling) non-zero element
...
\end{equation}
\NStodo{Explain how this is happening, why derivatives are relevant}
...
$S_{d+1}$ is Deligne's interpolated symmetric group category, but instead of the permutation representation as the generator we use the irreducible representation corresponding to the partition $(d,1)$ which is the permutation representation minus the trivial representation.  \nn{Add reference to https://arxiv.org/pdf/1212.2596.pdf}  Since this example is not the main thrust of this paper we will delay the details of its trivalent ribbon presentation to a future paper.
...
If $v$ is not a $10$th or $12$th root of unity \nn{and the square root
exists}, then the crossing acts
...
\DPTtodo{This is all written in terms of evaluation functors, which
  are not at all the way the conjectures are written}
...
$k$ points as the leaves, and form the $(k-2)!$ trees (still unembedded) obtained by the symmetric group action on the leaves. \nn{Explain why?} For each such unembedded tree, pick an embedding in the plane, with over- and under-crossings chosen arbitrarily.

The first line of the proposed basis $\cB^{\text{braided}}_6$ above comes from the partitions $6=2+2+2$,
...
inner product is nondegenerate, this observation does not lead to new relations.)
...
\nn{We should discuss the pentapent \diagram{0.1}{55f53ebaabfb09b3d4727e4b45c3637b0ed8f84c} and \diagram{0.1}{c44c9c3e568ce2218151df6d19f09681f6a6e4d8} below. In particular, replacing the last five diagrams with these five diagrams doesn't give a linearly independent set! You can replace four, however...}
...
\nn{By computing simultaneous eigenspaces for ..., we can verify the table in Dylan's section, and give explicit formulas for the idempotents. From these,
we can calculate the quantum dimensions of all the summands of $\mathfrak{g}^{\otimes 3}$.}
...
By direct calculation we find this triple of operators has distinct eigenvalues, so we obtain 80 eigenvectors, which are uniqely determined up to scale. In fact, we expect these to be matrix units (up to some scalars) with respect to the multiplication, but can't check this directly. \nn{..}
