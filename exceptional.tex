...
recall Vogel's \NStodo{We say Vogel some places and Cohen-De Man others} conjectural skein theoretic description of Deligne's classical
...
In addition, Cvitanović and Vogel \NStodo{and Cohen-de Man?} independently observed that the
...
So long as a certain finite set of polynomials in $\lambda$ are invertible in $R$, the space of $n$-boundary point diagrams in $\mathrm{Exc}_{R,\lambda}$ is non-zero.  In particular, any two ways of applying the above relations to a closed diagram to return a rational function in $\lambda$ will yield the same rational function.
\end{conjecture}

\begin{conjecture}[Classical sufficiency]
  \label{conj:class-suffic}
So long as a certain finite set of polynomials in $\lambda$ are invertible, the space of $n$-boundary point diagrams in $\mathrm{Exc}_{R,\lambda}$  is free of finite rank over $R$ for every $n$.  Furthermore, $\mathsf{Exc}_{R,\lambda}(n,m)$ has dimension bounded above by $1,\allowbreak0,\allowbreak1,\allowbreak1,\allowbreak5,\allowbreak16,\allowbreak80$
for $n+m=0,1,2,3,4,5,6$, respectively.  In particular, any closed diagram can be simplified using the above skein relations to a scalar, any $2$-boundary point diagram can be simplified to a scalar multiple of the strand, and any $3$-boundary point diagram can be simplified to a multiple of the trivalent vertex.
\end{conjecture}

\NStodo{Do we really care about the $16$ and $80$ here?}
...
These conjectures are a strengthening of a the conjecture of Deligne (as improved by Cohen--De Man \NStodo{others too?}), and would give a construction of an exceptional series passing through each of the exceptional Lie algebras.

\begin{conjecture}[Deligne's conejcture]
There exists
  \begin{itemize}
...
\item $\mathsf{Except}_{R,\lambda}(n,m)$ has dimension $1,\allowbreak0,\allowbreak1,\allowbreak1,\allowbreak5,\allowbreak16,\allowbreak80$
...
\begin{definition}
Let $R$ be a ring and $v$ and $w$ invertible elements of $R$ such that $[\lambda]$, $[\lambda+1]$, and $[1]$ are invertible. 
...
\begin{align*}
  ...
  t &= \{1\}\bigl(\{1\}\{2\lambda -1\} + (v^4 - v^2 - 1 - v^{-2} + v^{-4})\bigr).
\end{align*}
To these skein relations, we add the \emph{quantum exceptional Jacobi relation}
\begin{equation}
v^{-3} \;
\drawcrossX
\;+ v \;
...
Although the quantum exceptional Jacobi relation looks like an analogue of the ordinary Jacobi relation with lower order terms, it is important to emphasize that it is only the right analogue of the Jacobi relation in the exceptional setting.  In particular, it does not hold for quantizations of non-exceptional Lie algebras.

In order to have a skein theory that deforms the classical exceptional family, we should also have a relation deforming the classical exceptional relation and a relation between the over-crossing and under-crossing deforming the fact that they are equal classically.   Surprisingly, such relations follow from the quantum Jacobi relation, so long as certain polynomials are invertible.  The following theorem thus justifies the above definition.

\begin{theorem}
Suppose \nn{specific collection of polynomials} are invertible in $R$, then the following relations hold in 
\end{theorem}
...
So long as a certain finite set of Laurent polynomials in $v$ and $w$ are invertible in $R$, the space of $n$-boundary point diagrams in $\mathrm{QExc}_{R,v,w}$ is non-zero.  In particular, any two ways of applying the above relations to a closed diagram to return a rational function in $v$ and $w$ will yield the same rational function.
\end{conjecture}

\begin{conjecture}[Quantum sufficiency]
  \label{conj:class-suffic}
So long as a certain finite set of polynomials in $v$ and $w$ are invertible, the space of $n$-boundary point diagrams in $\mathrm{QExc}_{R,v,w}$ is free of finite rank over $R$ for every $n$.  Furthermore, $\mathsf{QExc}_{R,v,w}(n,m)$ has dimension bounded above by $1,\allowbreak0,\allowbreak1,\allowbreak1,\allowbreak5,\allowbreak16,\allowbreak80$
for $n+m=0,1,2,3,4,5,6$, respectively.  In particular, any closed diagram can be simplified using the above skein relations to a scalar, any $2$-boundary point diagram can be simplified to a scalar multiple of the strand, and any $3$-boundary point diagram can be simplified to a multiple of the trivalent vertex.
\end{conjecture}
...
The following theorems justify the above conjectures.  Under some mild assumptions, any trivalent ribbon category where the $n$-box space has dimension at most $1,0,1,1,5$ for $n=0,1,2,3,4$ satisfies a version of relation~\eqref{eq:quant-except-spec}.  Furthermore, the QEJacobi relation implies a certain deformation of the Vogel exceptional relation, and a relation between the overcrossing and undercrossing.  
