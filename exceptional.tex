...
\DPTtodo{This doesn't seem consistent with Deligne and Cohen-de Man.}
...
\begin{proof}
\DPTtodo{Add claim: trivalent vertex is rotationally symmetric}
...
\begin{theorem}
  ...
  Then, for some choice of~$v$, either $Q$ vanishes on all trivalent
  graphs or it satisfies the quantum exceptional relation
  \eqref{eq:quant-except}.
\end{theorem}
...
\begin{proof}
  ...
  eigenvalues for $R$ are $v^{-4}$, $\omega v^{-4}$, and
  $\omega^{-1} v^{-4}$, where $\omega$ is a primitive cube root of
...
\twostrandid
\; + v^{4} \;
 \cupcap \;\right]=0.\label{eq:quant-except-a}
  \end{equation*}

  If $\beta \ne 0$, we are done. If $\beta = 0$, the relation reduces
  to the Kauffman bracket relation for the Jones polynomial.
\DPTtodo{Prove that trivalent vertices are necessarily trivial or the golden
  category}
\end{proof}
