...
necessary.)  Let
\[
  \Psi_n = \prod_{\zeta: (\pm \zeta)^n = 1 \text{ and } (\pm \zeta)^m \neq 1 \text{ for $m < n$}}
  (v^{\frac{1}{2}}-\zeta v^{-\frac{1}{2}}).
\]
In terms of the usual cyclotomic polynomials we have if $n$ is odd
then $\Psi_n = \Psi_{2n} = v^{-\varphi(n)}
\Phi_n(v) \Phi_{2n}(v)$, but if $n$ is a multiple of $4$ then
$\Psi_n = v^{-\frac{\varphi(n)}{2}} \Phi_n(v)$. In particular, when $n$ is odd $\Psi_n$ vanishes exactly at primitive $n$-th and primitive $2n$-th roots of unity, while when $n$ is a multiple of 4, $\Psi_n$ vanishes exactly at primitive $n$-th roots of unity.
The notation $\{k\lambda + n\}$ can be viewed as another case of this:
for $n$ odd,
$\{n\} = \Psi_{4n}(v)$. For convenience, the values of $\Psi_n$ that
appear in this paper are give below.
\begin{align*}
  \Psi_1 = \Psi_2 = [1] &= v - v^{-1}\\
  \Psi_3 = \Psi_6 = \frac{[3]}{[1]} &= v^2 + 1 + v^{-1}\\
  \Psi_4 = \{1\} &= v + v^{-1}\\
  \Psi_5 = \Psi_{10} = \frac{[5]}{[1]} &= v^4 + v^2 + 1 + v^{-2} + v^{-4}\\
  \Psi_8 = \{2\} &= v^2 + v^{-2}\\
  \Psi_{12} = \frac{[6][1]}{[3][2]} &= v^2 - 1 + v^{-1}\\
\end{align*}
...
$[\lambda]$, $[\lambda-1]$, $\Psi_1$, and $\Psi_{12}$
...
where
\begin{align*}
  d &= -\frac{\Psi_8 [\lambda+5][\lambda-6]}{[\lambda][\lambda-1]}\\
...
\begin{equation}
\label{eq:QESquare-w}
\ngon[45]{4} + \frac{-\Psi_4\Psi_{12}^2[\lambda][\lambda-1]}{\Psi_1^2} \braidcross + \frac{x_1}{\Psi_1} \drawI + \frac{x_2}{\Psi_1} \drawH + \frac{x_3}{\Psi_1^2} \cupcap + \frac{x_4}{\Psi_1^2} \twostrandid =0
\end{equation}
...
\braidcross - \invbraidcross + \frac{\Psi_1}{\Psi_{12}} \left[\; \drawI \; - \; \drawH \; \right] + \frac{\Psi_1 [\lambda][\lambda-1]}{\Psi_{12}} \left[\; \twostrandid \; - \; \cupcap \; \right] = 0
\end{equation}
...
terms of $v$, what do the semidirect products mean. We should probably
...
There is a dense open subset $U \subset \mathbb{C}$ for which the idempotent
completion of $\mathrm{QExc}_{\mathbb{C},v,w}$, for $v,w \in U$, is
semisimple.
\end{conjecture}
...
\neq 1$.  Then $\Psi_8 = \{2\}$, $\alpha$, and $b+[3]\alpha$ are
...
\begin{equation*} \ngon[45]{4} + \frac{\alpha \Psi_{12} (b+[3]\alpha)}{\Psi_1 \Psi_3 \Psi_8}  \braidcross + \frac{y_1}{\Psi_3 \Psi_8}  \drawI + \frac{y_2}{\Psi_3 \Psi_8}  \drawH + \frac{y_3}{\Psi_1 \Psi_3 \Psi_8} \cupcap + \frac{y_4}{\Psi_1 \Psi_3 \Psi_8} \twostrandid =0
\end{equation*}
...
\begin{equation*}
\braidcross - \invbraidcross + \frac{\Psi_1 \Psi_3 \Psi_4 \Psi_8}{b+[3]\alpha} \left[\; \drawI \; - \; \drawH \; \right] - \frac{\Psi_1^2 \Psi_3 \Psi_4 \Psi_8 \alpha}{b+[3]\alpha} \left[\; \twostrandid \; - \; \cupcap \; \right] = 0
\end{equation*}
\end{itemize}
...
$\mathrm{QExc}_{\mathbb{C},v,w} \rightarrow \cC$.
\end{corollary}

This corollary justifies our definition of $\mathrm{QExc}$, since if the
...
$\mathrm{QExc}_{\mathbb{C},v,w}$ and the corresponding $2$-variable knot
polynomial.
...
We consider what happens when $v$ specializes to a $10$th or $13$th root of
...
\begin{equation*} [3] \ngon[45]{4} + \frac{\alpha \Psi_{12} (b+[3]\alpha)}{\Psi_8} \braidcross + \frac{y_1 \Psi_1}{\Psi_8} \drawI + \frac{y_2 \Psi_1}{\Psi_8} \drawH + \frac{y_3}{\Psi_8} \cupcap + \frac{y_4}{\Psi_8} \twostrandid =0
\end{equation*}
where the $y_i$ are Laurent polynomials in $v$ and $w$ given in Appendix \ref{app:coefficients}.
  \item
The quantum exceptional crossing relation:
\begin{equation*}
\frac{\Psi_{12} \alpha (b+[3]\alpha)}{\Psi_8} \left(\braidcross - \invbraidcross \right) + \Psi_1 \Psi_3 \Psi_4 \Psi_{12} \alpha \left[\; \drawI \; - \; \drawH \; \right] - \Psi_1^2 \Psi_3 \Psi_4 \Psi_{12} \alpha^2 \left[\; \twostrandid \; - \; \cupcap \; \right] = 0
\end{equation*}
\end{itemize}
 \end{proposition}
...
When $v$ is not a $10$th or $12$th root of unity we can divide by $[3]$,
$\alpha$, and $b+[3]\alpha$.  All that remains in order to prove Theorem
...
\item
$\cC$ is  $\mathrm{Rep}(S_3)$, and there are three distinct QEJacobi
relations: \NStodo{Need to prove somewhere that $SO(3)_{\zeta_{12}}$ plus the
symmetric braiding relation is actually $\mathrm{Rep}(S_3)$ without needing to
...
primitive $4$th, $5$th, $10$th, or$12$th root of unity, then $b+[3]\alpha$ is
non-zero.
\end{lemma}
\begin{proof}
If $v$ is a sixth root of unity then $b+[3]\alpha = b \neq 0$.  So our
assumptions imply that $v$ is not a $10$th or $12$th root of unity.  Suppose
for the sake of contradiction that $b+[3]\alpha = 0$.  Then by Proposition
\ref{prop:square-crossing} we have that $\cC$ satisfies the assumptions of
Lemma \ref{lem:IequalsH}.  Since $v$ is not a $12$th root of unity, the only
...
\frac{\Psi_{12}}{v^4-1+v^{-4}}$ which is non-zero since is not a
...
is non-zero.  By Lemma \ref{lem:bplus3alpha} we have that $b+[3]\alpha$ is
...
\nn{it feels like there's a discontinuity in the text here}
...
$(G_2)_{q^{-1}}$.  In these examples the above five diagrams are indeed
...
$-1$, $w^2$, and $\frac{v^2}{w^2}$ where $w$ is given by the formula from the
introduction.
\end{lemma}
\begin{proof}
First note that $\frac{1}{d} \cupcap$ and $\frac{1}{b} \drawI$ are projections
and are eigenvectors for the crossing with eigenvalues $v^{12}$ and $-v^{-6}$.
To find the remaining three eigenvalues we look at the action of the crossing
...
ones.  Namely, suppose $v$ is an eigenvector in the image of $\twostrandid
-\frac{1}{d} \cupcap -\frac{1}{b} \drawI$ with eigenvalue $(\beta, \gamma)$.
QEJacobi says that:
$$v^{-3} \beta \gamma -v^{-1} \gamma + \alpha \beta + \alpha v^{-4} = 0.$$
QECrossing says that:
$$\beta-\beta^{-1} - \frac{\Psi_1 \Psi_3^2 \Psi_4 \Psi_8}{b+[3]\alpha} \gamma - \frac{\Psi_1^2 \Psi_3 \Psi_4 \Psi_8 \alpha}{b+[3]\alpha} = 0.$$
...
\diagram{0.1}{8a27a57e82333b072ffb4d5b78de022da5c0852b}. This will
...
element of $\mathcal{Q}_4$. \nn{$\mathsf{Tangle}$ has not been defined.} In
...
$$\xi(3_1) = ...$$
which at \nn{$w = ...,  v= ...$} gives \nn{...}
One might also try to use the adjoint representation skein theory for $G_2$ in Kuperberg's paper \cite{MR1403861}; but there are many errors in those formulas: \nn{say what they are for future reference?} \nn{remove this sentence entirely?}
...
\section{When \texorpdfstring{$v$} is a small root of unity}
...
The following lemma is a mild generalization of \nn{Noah: what did you intend to cite here? \cite[Theorem 3.4]{1202.4396}?}where we have relaxed the non-degeneracy assumption.
...
\end{lemma}
\begin{proof}
If the two Temperley-Lieb diagrams are linearly dependent, then attaching a trivalent vertex shows that the trivalent vertex is zero which is a contradiction.  So we must have a relation of the form:
...
\begin{remark}
Strictly speaking in \cite{MR3624901} we only considered non-degenerate trivalent categories, but this can be avoided as follows.  The argument that the pentagon reduces still applies, so all small faces reduce, and non-elliptic diagrams span all $\cC_k$.  If the $10$ non-elliptic diagrams in $\cC_5$ are independent then Kuperberg \cite{MR1265145} says that $\cC$ is a quotient of $(G_2)_q$.  If there's a relation between these ten diagrams, the relation must be negligble.  By \cite{MR3624901} either there's a single relation and the category is a quotient of $(G_2)_{\zeta_{10}}$, or the non-degenerate quotient of $\cC$ is an $ABA$ category which contradicts $\cC$ being braided.
\end{remark}
...
see that $\cC$ is $\mathrm{Rep}(S_3)$ (which satisfies the latter relation for
...
because it has a quotient to  $(G_2)_{\zeta_12}$, and they also can't define
can't define $(G_2)_{\zeta_12}$ because it has braided quotient maps to both
$(G_2)_{\zeta_12}$ and $(G_2)_{-\zeta_12}$.  So the content of this conjecture
...
Again $\cH$ has braided quotient maps to both $(G_2)_{\zeta_12}$ and
$(G_2)_{-\zeta_12}$ (which are the same as categories but not braided
...
that these relations collapse all the way down to $\mathrm{Rep}(S_3)$ with
...
The first step is constructing the conjectured functor from tangles $\Qeval$,
...
$\mathsf{TriTangle}$ to $\mathsf{Jac}_{R[[h]],d,b}$. \nn{are we using
\texttt{mathrm} or \texttt{mathsf}?} The variable $h$ is used to keep track of
