...
through each of the exceptional Lie algebras.\DPTtodo{Not sure what
  this last sentence is doing}
...
There exists
  ...
  \item a symmetric monoidal functor $\eval$ from
    ...
    is the identity on objects and surjective on morphisms and sends
    the trivalent vertex to the generator of the $3$-boundary point
    space.
    \DPTtodo{This is assuming the 3-bdy point space is rank 1}
  \end{itemize}
...
$\mathsf{QExc}^D$ has the following properties.
\begin{itemize}
\item The idempotent completion of
  $\mathsf{QExc}^D_{\mathbb{C},v,w}$ is semisimple for \dots
  \DPTtodo{Need to modify this, don't have a map to $\mathbb{C}$}
...
for \dots and $n+m=0,1,2,3,4,5,6$, respectively.
\item If $\lambda$ is one of the special values in
  Table~\ref{tab:lambda-group}\DPTtodo{EDIT}, then the idempotent
  completion of the
  quotient of
  $\mathsf{Exc}^D_{\mathbb{C},\lambda}$ by negligible elements is
  equivalent to the category of representations of the corresponding
...
\DPTtodo{We have a different $s$ in Lemma~\ref{lem:constants}}
...
\begin{lemma} \label{lem:constants}
  ...
  \DPTtodo{Does $s$ here agree with earlier conventions?}
  \begin{equation}
