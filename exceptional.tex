...
\begin{table}
  \begin{tabular}{clc}
    \toprule
    $\lambda$ & Supergroup & v \\ \midrule
    $-5$ & Trivial group & \\
    $-4$ & $\mathrm{OSP}(1|2)$ & \\
    $-3$ & $\mathrm{PSL}(2)$ & $q^{\frac{1}{3}}$\\
    $-2$ & $\mathrm{PSL}(3) \rtimes \ZZ/2$ & $q^{\frac{1}{2}}$ \\
    $-3/2$ & $G_2$ & $q^2$ \\
    $-1$ & $\mathrm{PSO}(8) \rtimes S_3$ & $q$ \\
    $-2/3$ & $F_4$ & $q^3$ \\
    $-1/2$ & $E_6 \rtimes \ZZ_2$ & $q^2$\\
    $-1/3$ & $E_7$ & $q^3$\\
    $-1/5$ & $E_8$ & $q^5$\\
    \bottomrule
  \end{tabular}
  \caption{Values of $\lambda$ and the supergroup with related category
    ...
    automorphisms.  The last column gives the relationship between our
    parameter $v$ and the usual quantum group parameter $q$, note $v = q^{-\frac{L}{\lambda}}$
    (see Section \ref{sec:QGConventions} for quantum group conventions).}
  \label{tab:lambda-group}
\end{table}
\NStodo{Fill in the missing two rows.  Decide whether to move the last column 
to a different table.  Work out what to say about conventions for quantum supergroups
which aren't in Sawin.}
