...
\begin{corollary}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category over $\mathbb{C}$ with $\dim{\cC_4} \leq 5$, suppose there exist $v$ and $\alpha$ satisfying the previous theorem and with $d$, $b$,$[6]$, and $[5]$ nonzero.  Then there's a $w \in \mathbb{C}$ and an evaluation functor $\mathrm{QExc}_{\mathbb{C},v,w} \rightarrow \cC$.
\end{corollary}
...
Suppose that $\dim \cC_5 \leq 5$, then \eqref{eq:quant-except} holds for some choice of~$v$ with $v^6 = s$.
\end{theorem}
