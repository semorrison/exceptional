...
\begin{theorem}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category over $\mathbb{C}$ with $\dim{\cC_4} \leq 5$, suppose there exist $v$ and $\alpha$ satisfying the previous theorem, with $d$, $b$ nonzero and $v^{10} \neq 1$, $v^{12} \neq 1$.  Then $\widetilde{\Phi}_8 = \{2\}$, $\alpha$, and $b+[3]\alpha$ are also non-zero, and we have the following relations:
...
\end{equation}
where the $y_i$ are Laurent polynomials in $v$ and $w$ from Appendix \ref{app:coefficients}.
