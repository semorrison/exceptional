...
\begin{proposition}
  ...
  relations~\eqref{eq:simple-rels} with $v^{20} \ne 1$, then the
  parameters satisfy
\DPTtodo{Equation for $t$ currently does not agree with old notes.}
  \begin{align}
...
\begin{proof}
  ...
  solving for~$b$. Equation~\eqref{eq:b-t-rel} follows from closing
  off Equation~\eqref{eq:quant-except} with a trivalent vertex with
  one free end and solving for~$t$.
\end{proof}
