...
The skein theory for $\mathrm{Jac}_{R,d,b}$ is somewhat poorly behaved
...
are invertible, then $R^5$ injects into $\mathrm{Exc}_{R,\lambda}(2,2)$ as
...
If Conjecture \ref{conj:class-suffic} holds and $\lambda$ is one of $$-5, -4, -3,
...
completion of $\mathrm{Exc}_{\mathbb{C},\lambda}$ is
semisimple.
\end{conjecture}
...
\DPTtodo{Modified speculation. Note there are denominators of
  $2\lambda-3$ in the Cohen--De Man formulas}
...
There exists
  ...
  \item a category $\mathsf{Except}_{R,\lambda}$ where the objects are
...
Deligne's conjecture for $\mathsf{Except}_{R,\lambda} =
...
\begin{remark}
  We have recently learned that Vogel has computational evidence that
  Deligne's conjecture is false (private communication). Note that it
  is possible for the quantum
  Conjectures~\ref{conj:quant-suffic} and~\ref{conj:quant-consist} to
  hold while the classical conjectures are fas.e.
\end{remark}
...
\begin{align*}
  \Psi_1 = [1] &= v - v^{-1} &
  \Psi_2 = \{1\} &= v + v^{-1}\\
  \Psi_3 = \frac{[3]}{[1]} &= v^2 + 1 + v^{-1}&
  \Psi_4 = \{2\} &= v^2 + v^{-2}\\
  \Psi_5 = \frac{[5]}{[1]} &= v^4 + v^2 + 1 + v^{-2} + v^{-4}&
  \Psi_6 = \frac{[6][1]}{[3][2]} &= v^2 - 1 + v^{-1}
\end{align*}
...
\begin{equation}
\label{eq:QESquare-w}
\ngon[45]{4} + \frac{-\Psi_2\Psi_6^2[\lambda][\lambda-1]}{\Psi_1^2} \braidcross + \frac{x_1}{\Psi_1} \drawI + \frac{x_2}{\Psi_1} \drawH + \frac{x_3}{\Psi_1^2} \cupcap + \frac{x_4}{\Psi_1^2} \twostrandid =0
\end{equation}
...
\begin{equation*} \ngon[45]{4} + \frac{\alpha \Psi_6 (b+[3]\alpha)}{\Psi_1 \Psi_3 \Psi_4}  \braidcross + \frac{y_1}{\Psi_3 \Psi_4}  \drawI + \frac{y_2}{\Psi_3 \Psi_4}  \drawH + \frac{y_3}{\Psi_1 \Psi_3 \Psi_4} \cupcap + \frac{y_4}{\Psi_1 \Psi_3 \Psi_4} \twostrandid =0
\end{equation*}
...
\begin{equation*} [3] \ngon[45]{4} + \frac{\alpha \Psi_6 (b+[3]\alpha)}{\Psi_4} \braidcross + \frac{y_1 \Psi_1}{\Psi_4} \drawI + \frac{y_2 \Psi_1}{\Psi_4} \drawH + \frac{y_3}{\Psi_4} \cupcap + \frac{y_4}{\Psi_4} \twostrandid =0
\end{equation*}
where the $y_i$ are Laurent polynomials in $v$ and $w$ given in Appendix \ref{app:coefficients}.
  \item
The quantum exceptional crossing relation:
\begin{equation*}
\frac{\Psi_6 \alpha (b+[3]\alpha)}{\Psi_4} \left(\braidcross - \invbraidcross \right) + \Psi_1 \Psi_3 \Psi_2 \Psi_6 \alpha \left[\; \drawI \; - \; \drawH \; \right] - \Psi_1^2 \Psi_3 \Psi_2 \Psi_6 \alpha^2 \left[\; \twostrandid \; - \; \cupcap \; \right] = 0
\end{equation*}
\end{itemize}
 \end{proposition}
...
\frac{\Psi_6}{v^4-1+v^{-4}}$ which is non-zero since is not a
