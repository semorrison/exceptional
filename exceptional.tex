...
an object $X$ with $\dim \cC_1 = 0$, $\dim{\cC_2} = 1$, and $\dim{\cC_3} = 1$, with a (unique up to rescaling) non-zero element $\tau \in \cC_3$ called `the trivalent vertex', such that the category is generated as a ribbon category by $\tau$.\NStodo{Do we really want nondegenerate?}
\end{definition}
...
\begin{theorem}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category that is not the golden category and $\dim{\cC_4} \leq 5$, then there exists $v, \alpha, b, d \in R$ such that the following relations hold.  
...
\nn{This $v$ and $\alpha$ are unique up to...}

\begin{theorem}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category that is not the golden category  with $\dim{\cC_4} \leq 5$, let $v$ and $\alpha$ be as in the previous theorem, and further suppose that $d$, $b$,$[6]$, and $[5]$ are nonzero.  Then $\widetilde{\Phi}_8 = \{2\}$, $\alpha$, and $b+[3]\alpha$ are also non-zero, and we have the following relations:
...
\begin{corollary}
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category that is not the golden category with $\dim{\cC_4} \leq 5$, let $v$ be as above, and further suppose that $d$, $b$,$[6]$, and $[5]$ are nonzero.  Then there's a $w \in \mathbb{C}$ and an evaluation functor $\mathrm{QExc}_{\mathbb{C},v,w} \rightarrow \cC$.
\end{corollary}
