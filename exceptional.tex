...
By rescaling the trivalent vertex, it is easy to see that $\mathrm{Jac}_{R,d,b}$ is equivalent as a spherical tensor category to $\mathrm{Jac}_{R,d,bx^2}$ for any invertible $x$ in $R$.  In particular, if $R = \mathbb{C}$ then $\mathrm{Jac}_{R,d,b}$ does not depend on the choice of $b$.  A standard normalization is $b=6$.  \NStodo{Should we change $6$ to $12$?}
...
\begin{corollary}[Classical specialization]
  \DPTtodo{Added missing signs for $\lambda$ values}
...
relatively prime.\NStodo{Someone check this isn't nonsense.}  When
...
necessary.)  Let
\DPTtodo{Product should be restricted to primitive roots of unity.
  The notation is pretty heavy for something used everywhere in the
  paper.}
\[
...
\begin{definition}
Let $R$ be a ring and $v$ and $w$ invertible elements of $R$ such that $[\lambda]$, $[\lambda-1]$, $\widetilde{\Phi}_1$, and $\widetilde{\Phi}_{12}$ are invertible. \SMtodo{write these out, just here}
...
\SMtodo{this is going to require quite a bit of unpacking: what is $q$ in terms of $v$, what do the semidirect products mean. We should probably include a later section and just refer forward for this.}
...
If $\lambda$ is one \SMtodo{make a table!} of $-5, -4, -3, -2, -3/2,1,2/3,1/2, 1/3, 1/5$, and $v$ is not a root of unity, then the quotient of $\mathrm{QExc}_{\mathbb{C},v,v^\lambda}$ by the negligible elements is equivalent to the category of representations of the quantum versions of trivial group, $O(1 | 2)$, $\mathrm{PSL}(2)$, $\mathrm{PSL}(3) \rtimes \mathbb{Z}/2$, $G_2$, $\mathrm{PO}(8) \rtimes \mathbb{Z}/3$, $F_4$, $E_6 \rtimes \mathbb{Z}/2$, $E_7$, and $E_8$, respectively.
\end{conjecture}




\NStodo{Make ``quantum versions" more precise}\SMtodo{say what little we can about $v$ a root of unity}
...
Suppose $(\cC, X, \tau)$ is a trivalent ribbon category over $\mathbb{C}$ and $\dim{\cC_4} \leq 5$, then there exists $v, \alpha, b, d, t \in R$ such that the following relations hold.  
...
When $b+[3]\alpha$ is non-zero we can make the change of variables
...
Proposition~\ref{thm:quant-except} are somewhat weaker than stated. In
...
\cupcap
\; + v^{-4} \;
 \twostrandid = 0.$$  Then $\cC$ is the golden category with $\dim X = \frac{1 + \sqrt{5}}{2}$ or its Galois conjugate.  The golden category has $\dim \cC_4 = 2$ and satisfies four versions of the above relation, namely:
 \begin{itemize}
...
The quantum exceptional crossing relation:
\begin{equation*}
[3](b+[3]\alpha) \left(\braidcross - \invbraidcross \right) + \widetilde{\Phi}_1^2 \widetilde{\Phi}_3^3 \widetilde{\Phi}_4 \widetilde{\Phi}_8 \left[\; \drawI \; - \; \drawH \; \right] - \widetilde{\Phi}_1^3 \widetilde{\Phi}_3^2 \widetilde{\Phi}_4 \widetilde{\Phi}_8 \alpha \left[\; \twostrandid \; - \; \cupcap \; \right] = 0
\end{equation*}
\end{itemize}
 \end{proposition}
...
$$DETERMINANT.$$  
By our assumptions none of those factors can vanish
Note that if $d+\{8\} = 0$, then $\frac{[5]b}{\alpha} = 0$ which implies that $v$ is a $10$th root of unity.

%The first three diagrams are linearly independent unless $\cC$ is the Golden category, which means that $v$ is a $30$th root of unity.  If there's a linear dependence with non-zero coefficient on one of the crossings, then by taking a suitable linear combination of this dependence with its rotation, we see that $\braidcross$ simplifies as a sum of $\twostrandid$, $\cupcap$, $\drawI$, and $\drawH$.  Thus $\cC$ is generated as a pivotal category by the trivalent vertex.  By considering QESquare we see that the square simplifies.  Thus by \cite{trivalent} we see that $\cC$ must be a quotient of $SO(3)_q$ or $G2_q$.
\end{proof}

\begin{remark}
The CASES correspond to CASES.
\end{remark}
...
\end{lemma}
\begin{proof}
First note that $\frac{1}{d} \cupcap$ and $\frac{1}{b} \drawI$ are projections and eigenvalues for $\braidcross$ and $\drawH$ with eigenvalues $v^{12}$ and $-v^{-6}$.  So now we can project onto the image of $\twostrandid -\frac{1}{d} \cupcap -\frac{1}{b} \drawI$ and consider the action there.

Start with QECrossing and multiply by a crossing, then use QEJacobi to remove the $\drawcrossX$ term, and QECrossing to remove the $\drawH$ term.  Multiply again by the crossing and then by the projection $\twostrandid -\frac{1}{d} \cupcap -\frac{1}{b} \drawI$ to get a cubic equation in the crossing.  By Lemma \ref{lem:lin-ind-w}, the crossing does not satisfy a quadratic equation, so this cubic polynomial is the minimal polynomial and its roots are all eigenvalues.  A tedious calculation shows that the roots of this cubic equation are exactly $-1$, $w^2$, and $\frac{v^2}{w^2}$.  
\end{proof}

\begin{remark}
Since $\braidcross$ and $\drawH$ commute, the eigenvectors for the former must also be eigenvectors for the latter.  The eigenvalues are $(v^{12}, b)$, $(-v^{-6},t)$, $(-1, [\lambda][\lambda-1])$, $(w^2, \frac{[\lambda][\lambda+2]}{[1]})$, and $(\frac{v^2}{w^2}, -\frac{[\lambda-1][\lambda-3]}{[1]})$.
...
\begin{remark}
Recall that $$\drawI\; - \; \drawH - \frac{1}{d-1} \left(\twostrandid - \cupcap \right) = 0$$ is the defining relation of $SO(3)_q$, so this means we completely understand the planar part of $\cC$ in that case.  However, the crossing need not lie in the planar part.
\end{remark}
