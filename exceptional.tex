...
\item Introduction
  ...
  \item Evaluations we can compute easily
    ...
    \item Knots and links with Conway graph with width $\le 3$,
      including all 11-crossing diagrams and probably all 12-crossing
      knots.
    \end{enumerate}
  \end{enumerate}
...
monogons, bigons, and triangles, and let us change framings along
...
\end{equation}
\begin{conjecture}
  The vector space over $\QQ(v,w)$ of framed, embedded trivalent
  graphs with $n$ free ends modulo the local skein relations in
  \eqref{eq:simple-rels-spec} and~\eqref{eq:quant-except-spec} has
  dimension $1,0,1,1,5,16,80$ for $n=0,1,2,3,4,5,6$, respectively, and
  is finite-dimensional for all~$n$. In particular, these relations
  suffice to evaluate any closed framed trivalent graph to a rational
  function in $v$ and~$w$.
\end{conjecture}
In fact these relations are quite general. With some mild assumptions
on the base ring, any skein theory where the $n$-box space has
dimension at most $1,0,1,1,5$ for $n=0,1,2,3,4$ satisfies
relation~\eqref{eq:quant-except-spec}. See
Theorem~\ref{thm:quant-except}.
