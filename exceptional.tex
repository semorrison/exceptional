...
Work of Deligne, Cvitanovic, Vogel, Cohen-de Man, and others, can be summarized as a
...
the morphism spaces from $n$ to $m$ consist of $R$-linear combinations planar
...
functor from $\mathrm{Jac}_{R,b,d}$, where we interpret each strand as a copy of
...
In addition, independently Cvitanovic and Vogel observed that the adjoint representations
of the exceptional Lie algebras satisfy another relation, the \emph{classical exceptional
relation}.


\begin{definition}
If $2+d$ is invertible in $R$, let $\mathrm{Exc}_{R,b,d}$ be symmetric tensor category which is the quotient of 
$\mathrm{Jac}_{R,b,d}$ modulo Vogel's exceptional relation.
\begin{equation}
\ngon[45]{4}\; = \;\drawI\; + \;\drawH\;
 + \frac{b^2-b}{2+d} \left[ \;\twostrandid\; + \;\cupcap\; + \;\symcross\; \right]
...
So long as a certain finite set of polynomials in $b$ and $d$ are invertible, the space of $n$-boundary point diagrams in $\mathrm{Exc}_{R,b,d}$  is free of finite rank over $R$ for every $n$, and the space of $0$-boundary point diagrams is rank $1$.  
\end{conjecture}
...
\begin{corollary}
If $d$ is one of ???, then the quotient of $\mathrm{Jac}_{\mathbb{C},b,d}$ by the negligible elements is ???, respectively.
\end{corollary}

\begin{conjecture}
There is a dense open subset of $\mathcal{C}$ for which the idempotent completion of $\mathrm{Jac}_{\mathbb{C},b,d}$ is semisimple.
\end{conjecture}

We expect that in order for the idempotent completion to be semisimple one needs to invert infinitely many polynomials.  For example, for $\mathrm{GL}_t$ in order for the idempotent completion to be semisimple you need to invert $t-n$ for every positive integer $n$.
...
conjectured by Deligne and Cohen--De Man.  

It follows from this conjecture that if $d$ is one of \todo{which values?} then the quotient of $\mathrm{Exc}_{R,b,d}$
...
lollipop, bigon, triangle, quantum anti-symmetry, quantum Killing symmetry, and quantum Jacobi
relations.
\end{definition}
...
\nn{Does $E_6$ explain one of the components of $\det M_5 = 0$? -- $[\lambda+2]$ vanishes at $\lambda = -1/2$, $\mu=1/2$, which according to the table below is $E_6 \rtimes \bbZ/2\bbZ$, which should have a quantum version.}

Although it may appear that new factors are appearing in the denominators as we look at $\det M_3, \det M_4, \det M_5, $ and $\det M_6$, one should recall that the bracket polynomials $[a \lambda + b]$ are not generally irreducible. The irreducible factors appearing in the denominator after cancellation remain the denominators \nn{already} introduced: $\det M_4$ has poles at $w=\pm1, v=\pm w, v^6+w^2 = 0$, and $1+v^4w^2 = 0$,
